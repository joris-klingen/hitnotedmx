...
1. **Expand the dynamic recipe library** — beyond the current 12: rain,
...
2. **Grid-aware chases (more directions)** — today's chases run mostly
...
3. **Full-colour chases (self-coloured recipes)** — recipes that carry their
   ...
   coloured one. Decide how these compose with the colour-fade stage. When
   these land, add the **Multicolor** group to the trigger menu (its slot is
   already reserved in `TriggerMenu::buildModel`).

4. **Calming "ambient" breathes for quiet sections** — very slow, smooth
...
5. **Confirm long-session GUI stability** — the preview now draws 72 cells
...
6. **Remove dead colour-pitch helpers** — `isPrimaryColorPitch` /
   `isSecondaryColorPitch` in `Composition.cpp` are unused (compiler warns).
   Trivial cleanup.

## Recently shipped (see STATUS.md for detail)

- **Pixel-density control (experimental spike)** — a master DENSITY knob
  (0–100%, host-automatable) that thins how many bar LEDs are lit via a
  stable per-pixel position hash, so dark rooms can run fewer LEDs without
  flicker and without dimming the pixels that stay on (full-brightness
  flashes preserved). Gate lives in `computeDmx`; spots untouched. Spiked
  for visual judgement — follow-ups if kept: tune the scatter (hash vs
  every-Nth) and whether 100% should be the default.
- **Trigger-menu dynamics regrouped** — the flat "Dynamics" block is now
  three feel-based groups: **Chases**, **Breathes**, **Wild**. Pitches
  unchanged (24–35); presentation only. A **Multicolor** group is reserved
  for the future self-coloured recipes (#3).
