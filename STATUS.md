...
| Dynamic recipes | `Recipes.{h,cpp}` | Four feel-groups, each a full chromatic octave starting on a C, all function-pointer dispatch (one table per group), 48 recipes total. Each table is ordered logically and matches the menu columns 1:1. **Chases C0 (24–35):** chase, ripple (also in Breathes — here at chase speed, a ring per beat), ping-pong, diag, radar, snake, theater, spiral, waves, expand, contract, chase H (whole columns sweeping across, one pass per beat — the lateral counterpart of chase; direction is the global Flip/Reverse notes now, not up/dn variants). Chases are **beat-locked by design**: chase/chase H/waves/expand/contract cycle per beat, ping-pong/diag/radar/spiral per 2 beats, snake one beat per bar-height. **Breathes C1 (36–47):** tide, sine, ripple, ripple H, bloom, halo, moon rise, soft ball, drift, aurora, shimmer, glow — the **periodic movers all cycle once per bar** at the bank rate (half the chase rate); the free-running textures (soft ball, aurora, shimmer, glow) keep their own slow drift. **Wild C2 (48–59):** strobe (root C, null slot — driver-level shutter), sparkle, sparkle few, lightning, glitch, fountain (moved from Chases; velocity = beat-speed here, fixed mid trail), rain, waterfalls, bounce, fast ball, pong (beat-synced here), burst. **Nebula and Borealis carry a brightness floor (~0.18)** — never black, so a breathe can mask holes into them instead. **Multicolor C3–C4 (60–83, two octaves, 24):** self-coloured `DynamicColorFn` recipes returning RGB directly — rainbow, comet, VU meter, VU smooth, fire, embers, magma, lava, heatmap, ocean, forest, desert (C3); sunset, twilight, borealis, night sky, galaxy, nebula, storm, plasma, police, disco, velvet, rouge (C4). `DynamicFn` carries a `tail` arg (0..1); `DynamicColorFn` carries a `param` arg (VU gain). Moving-head chases render a velocity-driven comet trail via `cometBrightness` / `cometBrightnessF`. |
| Composition | `Composition.{h,cpp}` | Bit-mask lookup tables, mask intersection across utility / static / dynamic layers, primary/secondary palette routing, spot RGBW with warm-white tint. Plus master-dim scaling and velocity-driven linear colour fade (`ColorFadeState`, see below). **Colour routing comes from the pixel-zone selectors (their velocity picks primary vs secondary); bar selectors set each bar's brightness ceiling (velocity / 127) and no longer route colour; brightness dynamics modulate brightness only** — so a chase takes the zone's colour, or primary by default. **Palette colour notes carry no brightness of their own — velocity sets only the fade duration, so a soft note rises slowly to full colour.** **Per-bank velocity semantics** (doc table at the top of the file): Chases → comet tail, Wild → beat-division 1/16..1/1 (except sparkle/sparkle few, free-running speed), Breathes → glow-mask depth (127 = the breathe untouched, softer notes dim it into a drifting patchy wash via `breatheGlowMask` — the old on/off density islands are gone) + half the chase rate, Multicolor → animation speed (except VU meter, beat-locked with velocity → gain). **White default:** a bar/pixel/dynamic/strobe trigger held with NO palette colour renders full white (matching click-preview). **Self-coloured Multicolor recipes (60–83) are the exception to colour routing:** when held, their RGB replaces the palette route for the lit pixels (multiple combine per-channel max); structural masks and brightness dynamics still gate/modulate on top, and the colour-fade stage doesn't apply to them. |
...
- **Chase velocity is always the tail** — chases no longer change what their
  velocity means when Master Speed (G8) is held. Previously `speedHeld` pinned
  the tail to a fixed `0.5` and re-tasked the velocity to pick the palette route
  (>=64 primary, <64 secondary), so a chase's trail went dead the moment Speed
  came in. Now the tail is `vel / 127` unconditionally and chase rate comes
  solely from the global speed multiplier — one meaning per velocity, no modes.
  **Wild and Breathes are untouched:** wild keeps route-by-velocity under Speed
  (it is now the only bank that does), breathes keep glow-mask depth. Route
  selection is otherwise unaffected — zone selectors still win over `dynRoute`.
  No mapping freeze: note→name is unchanged (`24 ch Chase` …), only velocity
  semantics moved; `mapping-frozen` + `recipe-range` green, both targets build.

- **Recipe tuning pass (mapping v12): Fountain → Wild, Chase H, floors, bump-through-freeze**
  — **Fountain moved to Wild** (53, replacing Static, which was removed — it
  added little): velocity is beat-speed there, so the trail is a fixed mid
  length. Its old Chases slot (35, B0) is **Chase H** — whole columns
  sweeping across the grid, one pass per beat, comet trail in bars; the
  lateral counterpart of Chase. Frozen as **`mappings/v12.tsv`** (two rows).
  **Nebula and Borealis got a brightness floor (~0.18)** so they never show
  black spots — mask holes into them with a breathe instead. And **Bump now
  punches through Freeze**: `computeDmx` saves a pre-overlay scene snapshot
  each unfrozen block, and the frozen path rebuilds the display from it with
  a fresh flash overlay advanced on the RAW (unpaused) clock — so accents
  fire and decay over a frozen look instead of being swallowed (previously
  the frozen early-return blocked section 9 entirely). The flash + overlay
  logic is factored into shared `advanceBumpFlash` / `applyBumpOverlay`.

- **Recipe-speed normalisation pass (mapping v11)** — the banks' default rates
  were scattered (halo 2 bars, tide 3, bloom 4, moon rise 8, drift 5, sine
  half a bar); now **chases cycle on the beat** (verified — chase/waves/
  expand/contract 1 beat, ping-pong/diag/radar/spiral 2 beats, snake one beat
  per bar-height, all beat-locked already) and the **periodic breathes all
  cycle once per bar** (tide, sine, ripple, ripple H, bloom, halo, moon rise,
  drift retuned; the free-running textures — soft ball, aurora, shimmer,
  glow — keep their drift). The Speed ladder gained a **0.5× bottom step**
  (vel 1–14 = 0.5× … 57+ = 8×), so vel 1 = a 2-bar breathe and vel 15 = the
  1-bar default. **Comets was removed and Ripple took its chase slot (25)**
  — same recipe as the breathe (38) but at chase speed, a ring per beat —
  frozen as **`mappings/v11.tsv`**. And the Breathes' velocity-density
  gating (`breatheDensityMask`, the islands) **never really worked and is
  gone**: velocity now blends a `glow`-style field over the breathe
  (`breatheGlowMask`) — 127 leaves it untouched, softer notes dim the shape
  into a drifting patchy wash, never gated, never fully dark.

- **Discrete Speed ladder + transport-locked recipe clocks** — Speed (G8)
  velocity was a continuous exponential (vel 64 = 1×, ~0.13×–3.9×), so "one
  bar per cycle" wasn't reachable on purpose; it's now a **discrete
  power-of-two ladder** (doubling every 14 velocity, capped at 8×: vel 1 =
  1× … 43+ = 8×), anchored on the breathes' 2-bar cycle — halo runs 2 bars /
  1 bar / 2 beats / 1 beat. And the chases/breathes' accumulated phase clocks
  now **re-lock to the song position when the host transport starts or jumps
  backwards** (loop wrap / relocate) via `BumpState::resyncClocks` — before,
  a negative ppq delta was clamped away and the movers just kept going, so a
  snake never restarted at the bottom on song start. Together the two make
  recipe cycles land on the bar grid.
