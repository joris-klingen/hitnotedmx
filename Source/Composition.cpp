                 ...
                 ColorFadeState* fade, double dtSeconds, float pixelDensity) noexcept
{
    ...
    // recomputing, so `out` keeps last block's values until freeze releases.
    // (The processor's beat clock keeps running, so releasing freeze resumes
    // in time with the song rather than where it was paused.)
    ...
    if (state.isActive (static_cast<std::uint8_t> (kFreezeNote)))
        return;   // hold the last composed frame untouched
